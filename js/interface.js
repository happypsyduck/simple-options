...
function getMarketInformation(){
  ...
  if(currentRound < 1 || currentRound > topRound){return;}
...
function refreshRoundInformation(){
	var skip = false;
	if(roundTransition == true){ skip = true; }
	if(topRound == 0) {skip = true; }
	if(ethereum.networkVersion !== targetNetwork){ skip = true; }
...
function adjustCountDown(){
  ...
  if(currentRound != topRound){return;} // Countdown only applies to current round
...
function gotoNextRound(){
  if(roundTransition == true){return;}
  if(currentRound < topRound){
